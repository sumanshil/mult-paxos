...
class Agent:
    ...
    def _send_to_all(
        ...
        return [reply_type.from_dict(reply) for reply in replies if reply is not None]
    ...
    def _main_loop(self, q: queue.Queue[Agent._QEntry]) -> None:
        while True:
            ...
            if isinstance(entry.message, ClientRequest):
                ...
                accepted = cast(list[Accepted], self._send_to_all(self._accept_url, accept, Accepted))
...
class Acceptor(Agent):
    ...
    def _handle_prepare(self, prepare: Prepare, future: Future[Message]) -> None:
        # Phase 1b, Fig. 3 in Chand.
        if prepare.ballot <= self._ballot_number:
            future.set_exception(ValueError("state prepare"))
            return
