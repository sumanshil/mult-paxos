...
class Node:
    ...
    def _send(self, node: str, url: str, message: Message,  reply_type: type):
        try:
            print("URL ==> " + f"http://{node}{url}")
            print("Payload ==> " + str(dataclasses.asdict(message)))
            reply = requests.post(f"http://{node}{url}", json=dataclasses.asdict(message))
        except requests.exceptions.RequestException as exc:
            logging.getLogger("requests").warning(exc)
        else:
            print("Reply ==> " + str(reply.content))
...
class Proposer(Node):
    ...
    def __next_proposal_number(self):
...
class Acceptor(Node):
    ...
    def _handle(self, message: Message):
        ...
        return PrepareOK(message.proposal_number, self._highest_accept, self._accepted_value)
