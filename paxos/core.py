...
class Coordinator(Node):
    ...
    def __init__(self, config: Config, port: int, prepare_url: str, accept_url: str):
        ...
        try:
            ...
            self._id = 1 + next(i for i, node in enumerate(config.nodes)
    ...
    def _handle(self, message: Message) -> ClientReply:
        ...
        with self._mutex:
            if self._highest_round != highest_round or self._value is not None:
                raise ValueError(f"round {highest_round} preempted by {self._highest_round}")
        ...
        # TODO: parallelize
        for node in self._config.nodes:
            if reply := send_message(node, self.accept_url, accept_message):
        ...
        with self._mutex:
            if isinstance(message, Prepare):
                ...
                if pn > self._highest_prepare:
                    ...
                    return PrepareOK(pn, self._highest_accept, self._accepted_value)
